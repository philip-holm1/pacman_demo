> **Generated snapshot:** This file is auto-generated from spec.md by $generatorRel on 2025-11-19 15:35:52. Do not edit this file directly.
...
- **FR-009**: Score calculation MUST account for active `ScoreMultiplier` powerups deterministically. If a player collects another ScoreMultiplier while one is active, the multipliers stack (e.g., x2 â†’ x4) and durations run independently.
- **FR-010**: High score MUST be stored locally on device (simple local persistence) and displayable from the main menu.

### Testing & Stability

- **FR-011**: The game MUST not deadlock or crash if a powerup spawns and a ghost/actor overlaps â€” spawning must avoid collisions.
...
- **SC-003**: Powerup visual timers accurately reflect remaining time within Â±0.5s for durations up to 20s in 20 tests.
